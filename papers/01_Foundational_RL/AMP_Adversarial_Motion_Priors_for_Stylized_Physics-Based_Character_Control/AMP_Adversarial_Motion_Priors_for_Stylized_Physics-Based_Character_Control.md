...
| **LSGAN** | Least Squares GAN | 最小二乘 GAN，AMP 实际实现用的判别器目标 |
| **Discriminator** | — | 鉴别器，判断数据真假 |
| **Style Reward** | — | 风格奖励，来自鉴别器输出 |
| **GP** | Gradient Penalty | 梯度惩罚，只作用在真样本上，防止判别器过拟合 |
...
| 阶段 | 样本量 | $D$(假) | $D$(真) | 风格奖励 $r^S$ | 任务奖励 $r^G$ |
|------|--------|---------|---------|----------------|----------------|
| **初期** | 0 | −1.0 | 1.00 | 0.00 | 0.12 |
| **学会站立** | 1000 万 | −0.55 | 0.92 | 0.40 | 0.32 |
| **基本移动** | 5000 万 | −0.15 | 0.68 | 0.67 | 0.62 |
| **收敛** | 2 亿 | 0.00 | 0.35 | 0.75 | 0.88 |

$r^S$ 不是另画一条线，而是把 $D$(假) 代入 LSGAN 式 $r^S = \max[0,\ 1 - 0.25(D-1)^2]$：$D=-1$ 时奖励和梯度一起消失，$D=0$ 时恰好是 **0.75**（对抗均衡），$D=1$ 时满分 1。任务奖励按论文 $w^S=w^G=0.5$ 的 Target Heading。
...
把这张表画成学习曲线，能看到对抗训练的典型形态——前期 $r^S$ 贴地（判别器碾压策略），中期 D(假) 往 0 爬、准确率从 99% 往下降，后期两边都难分、任务分兑现。真正训练时 TensorBoard 上不止这两条；logit、准确率、梯度惩罚怎么读、坏了是什么病，见 [训练曲线怎么读](#amp-curves-guide)。

<div class="mermaid">
xychart-beta
    title "Target Heading 风格奖励（示意）"
    x-axis "样本（百万）" [0, 10, 50, 200]
    y-axis "r^S" 0 --> 1
    line [0.00, 0.40, 0.67, 0.75]
</div>
...
## 📈 训练曲线怎么读 {#amp-curves-guide}

算法侧直接复用 PPO，所以 KL / clip / 熵怎么读见 [PPO 笔记的训练曲线](../PPO_Proximal_Policy_Optimization/PPO_Proximal_Policy_Optimization.html#ppo-curves-guide)——AMP 把 clip 收成 **0.02**，因为风格奖励本身在动。这篇每天多盯的是**判别器侧**那几条：风格奖励、任务奖励、$D$ 的 logit、真假准确率、梯度惩罚。多数线不能用「越高越好」打发——$r^S$ 一上来就 0.75 往往是判别器崩了，准确率长期 99% 是策略拿不到梯度。下面这个实验台把 Target Heading 的示意曲线做成六种病历：先点「健康」记住标准形态，再点坏掉的那几种，看是哪条线先报警。

<div class="paper-demo" data-demo="amp-curves"><p class="demo-fallback">（本节含交互演示，需要启用 JavaScript）</p></div>

### 方向速查表

带 \* 的「好方向」有前提：奖励权重没写歪、回放你真的看过。表格里的健康终点按上面第 4 步 Target Heading（$w^S=w^G=0.5$）。IsaacGymEnvs / MimicKit 的列名见折叠块「名字对照」。

| 曲线 | 好方向 | 健康形态 | 立刻要查的坏样子 |
|------|--------|----------|------------------|
| **风格奖励 $r^S$** | 从 0 爬到 ~0.75\* | $D$(假) $-1\to 0$ 代入 LSGAN 式：0.00 → 0.40 → 0.67 → **0.75** | 长期 ≈ 0（奖励消失）；一上来就 0.75（假饱和） |
| **任务奖励 $r^G$** | 越来越高\* | 0.12 → 0.32 → 0.62 → **0.88** | 横盘在 ~0.12；涨了但回放动作丑 |
| **$D$(假) / $D$(真)** | **看均衡，不是越高越好** | 假：−1 → 0；真：1 → ~0.35 | 假钉在 −1；真假都塌到 0 |
| **agent / demo 准确率** | 从接近 1 降下来 | 前期 ~99%，后期 ~55% / ~70% | 长期 99%；长期两边 ≈ 50% |
| **梯度惩罚** | **看区间** | 原始 $\lVert\nabla D\rVert^2$ 落在 ~1（大约 0.6–1.6） | 持续高于 4；长期 ≈ 0 |
| **速度覆盖** | 本任务：1–5 m/s 铺开 | 后期 ~0.94 | 卡在 ~0.40（只会走路） |

> ⚠️ **符号陷阱**：$r^S$ 在 $D=0$ 时是 0.75 不是 1，别把「没到满分」当成没学会。IsaacGymEnvs 的 `disc_agent_acc` 是「把策略样本判成假」的比例（logit < 0），越高说明判别器越碾压策略，**不是越高越好**。

### 先看哪几条，再怎么交叉验证

优先级从高到低只有三层：

1. **风格信号在不在**：$r^S$ + $D$(假)。这条贴地或假饱和，后面全是噪声。
2. **对抗有没有在打**：准确率 + 梯度惩罚。回报一旦异常，先来这里找原因。
3. **任务做没做、会不会只会一种动作**：$r^G$ + 速度覆盖 + 回放。它们揭穿「总分好看但怪走」和「只会数据集里最好学的那种」。

<div class="mermaid">
flowchart TB
    R{"r^S 贴地 / 假饱和 / 在涨？"}
    R -->|贴地 ≈ 0| A{"D(假) 是否钉在 −1？"}
    A -->|是| Strong["判别器过强：加大 GP"]
    A -->|D 也在乱跳| Ppo["去看 PPO 的 KL"]
    R -->|一上来就 ~0.75| W{"准确率两边都 ≈ 50%？"}
    W -->|是| Weak["判别器过弱：减小 GP"]
    W -->|不是| Check["先看 D 的均值"]
    R -->|看起来在涨| Vid{"打开回放：步态多样吗？"}
    Vid -->|只会一种走| Coll["模式崩塌"]
    Vid -->|任务成了但动作丑| Task["任务压垮风格"]
    Vid -->|很像但跟不住 v*| Style["只有风格"]
    Vid -->|是| OK["健康：D(假)→0"]
</div>
...
<h3 id="曲线-风格奖励">风格奖励 $r^S$（对抗均衡是 0.75 不是 1）</h3>

论文实现：

$$r^S = \max\big[0,\ 1 - 0.25\big(D(s,s') - 1\big)^2\big]$$

第 4 步那张表和上面实验台用的是同一组锚点，由 $D$(假) 直接算出来：

| 样本（百万） | 0 | 10 | 50 | 200 |
|--------------|---|----|----|-----|
| $D$(假) | −1.0 | −0.55 | −0.15 | 0.00 |
| $r^S$ | 0.00 | 0.40 | 0.67 | 0.75 |

- **好方向**：从 0 爬到约 0.75。前期贴地是判别器碾压策略（$D\le -1$ 时 $\max(0,\cdot)$ 把奖励和梯度一起掐掉），中期策略追上，后期停在均衡。
- **长期 ≈ 0**：判别器过强，策略拿不到风格梯度。先看 $D$(假) 是不是钉在 −1，再调 `disc_grad_penalty`。
- **一上来就 0.75**：不是「开局满分」，是 $D\approx 0$——判别器已经不会打分。看准确率是不是两边都 50%。
- **涨得又快又稳，视频却难看**：进入「模式崩塌」或「任务压垮风格」，这条曲线单独看会给出假阳性。

<h3 id="曲线-任务奖励">任务奖励 $r^G$（Target Heading 才有）</h3>

$$r^G = \exp\big(-0.25(v^{\ast} - v _ {x\mathrm{com}})^2\big)$$

- **好方向**：0.12 → 0.32 → 0.62 → 0.88。和风格分一起涨，才是「用对的风格完成任务」。
- **横盘在 ~0.12**：策略还是随机的。查任务项有没有接到奖励合成（MimicKit 默认 `task_reward_weight: 0.0`）。
- **涨到 0.9、风格分却贴地**：任务压垮风格，$w^G$ 太大。
- 纯风格实验（spinkick）不要硬造一条任务分——用 $r^S$ 和回放就够。

<h3 id="曲线-判别器logit">$D$(假) / $D$(真)（对抗的两极）</h3>

LSGAN 真样本目标 +1、假样本目标 −1。Nash 均衡在 **0**：两边都骗不过对方。

- **假：−1 → 0，真：1 → ~0.35**。真样本降得慢一些，因为动捕比策略干净。
- **假钉在 −1**：判别器过强。和 $r^S\approx 0$ 是同一件事。
- **真假都塌到 0**：判别器过弱。此时 $r^S$ 会假饱和在 0.75。
- IsaacGymEnvs 日志名是 `info/disc_agent_logit` / `info/disc_demo_logit`，就是这两条。

<h3 id="曲线-判别器准确率">agent / demo 准确率（越高越说明在碾压）</h3>

IsaacGymEnvs：`disc_agent_acc = mean(logit < 0)`，`disc_demo_acc = mean(logit > 0)`。

- **好方向**：从 ~99% 降到对抗均衡附近（agent ~55%，demo ~70%）。前期高是正常的。
- **长期 99%**：判别器过强，策略的每一步都被判假。
- **长期两边 ≈ 50%**：判别器过弱，抛硬币。
- **demo 一直 95%+、agent 也在降**？少见；更常见的是任务压垮风格时两条都降不下来——策略根本没在模仿。

<h3 id="曲线-梯度惩罚">梯度惩罚（新动作最该调的旋钮）</h3>

论文只对**真样本**做 $\lVert\nabla_\psi D\rVert^2$ 惩罚（不是 WGAN-GP 的插值点），权重 $w _ {GP}=10$（MimicKit 默认 5）。日志里的 `disc_grad_penalty` 一般是惩罚**之前**的原始梯度平方。

- **好方向**：原始值落在大约 $[0.6,\ 1.6]$。
- **持续高于 4**：判别器太尖，GP 系数太小。IsaacGymEnvs 文档写明：换新动作时**最先调**的就是 `disc_grad_penalty`，典型范围 $[0.1,\ 10]$，先从大的试。
- **长期 ≈ 0**：判别器被抹平，接着就会看到真假 logit 塌到 0。
- 回放池（论文 $10^5$，MimicKit 20 万）和 GP 一起防过拟合；GP 再对、池子关了也会过强。

<h3 id="曲线-速度覆盖">速度覆盖（揭穿模式崩塌）</h3>

Locomotion 混合数据集的 $v^{\ast}\in[1,5]$ m/s。把策略能跟住的速度区间占 $[1,5]$ 的比例记下来。

- **好方向**：后期铺到 ~0.94，并且在 $v^{\ast}\approx 2.4$ 附近能看到步态切换（上面那个风格 × 任务演示的右图）。
- **卡在 ~0.40**：只会走路。$r^S$ 可以看起来健康——这就是模式崩塌。
- 单速度任务（Zombie / Stealthy 固定 1 m/s）不要看这条，看回放里的姿态。

<h3 id="曲线-名字对照">名字对照：你日志里那一列到底是谁</h3>

| 本文称呼 | IsaacGymEnvs | MimicKit |
|----------|--------------|----------|
| 风格奖励 $r^S$ | `info/disc_reward_mean` | `_calc_disc_rewards()`（× `disc_reward_scale`） |
| 任务奖励 $r^G$ | 任务项，需自己记 | `task_r`；默认权重 0 |
| $D$(假) | `info/disc_agent_logit` | `disc_agent_logit` |
| $D$(真) | `info/disc_demo_logit` | `disc_demo_logit` |
| agent 准确率 | `info/disc_agent_acc`（logit < 0） | `_compute_disc_acc()` |
| demo 准确率 | `info/disc_demo_acc`（logit > 0） | 同上 |
| 梯度惩罚 | `info/disc_grad_penalty` | `disc_grad_penalty` |
| 判别器 loss | `losses/disc_loss` | `disc_loss` |
| KL / clip / 熵 | 标准 PPO | 见 PPO 笔记；AMP 的 clip = **0.02** |

MimicKit 默认日志比 IsaacGymEnvs 瘦：源码把 `disc_loss` 打进 `disc_info`，但速度覆盖、分项奖励要自己记，否则「模式崩塌 / 任务压垮风格」要等回放才看得见。
...
- $D$(假)：−1.0 → −0.55 → −0.15 → **0**；$r^S$：0.00 → 0.40 → 0.67 → **0.75**（第 4 步那张表）。
- $D$(真) 从 1 降到 ~0.35；agent 准确率从 99% 降到 ~55%。
- $r^G$ 同步爬到 ~0.88；GP 贴在 ~1；速度覆盖铺开。
- **什么都不用改。** 打开回放确认步态，存 checkpoint。

<h3 id="病历-判别器过强">② 判别器过强</h3>

- **先兆**：$D$(假) 钉在 −1，agent 准确率 99%，GP 原始值冲到 4 以上。
- **发作**：$r^S$ 被 $\max(0,\cdot)$ 压成 0，策略拿不到风格梯度，任务分也涨不动。
- **原因**：`disc_grad_penalty` 太小、判别器 lr 比策略快太多、回放池没开导致过拟合当前策略。
- **处理**：把 GP 系数往 5–10 调（论文 Table 4 是 10，MimicKit 默认 5）；确认 `disc_buffer_size` 还在。已经训歪的 checkpoint 回滚比硬拉回来容易。

<h3 id="病历-判别器过弱">③ 判别器过弱</h3>

- $r^S$ 一上来就 **0.75**，看起来像学会了。
- 真假 logit 都塌到 0，准确率两边 ≈ 50%，GP ≈ 0。
- **原因**：GP 系数太大，判别器被抹平，不会打分。策略拿到的是常数奖励。
- **处理**：减小 `disc_grad_penalty`。IsaacGymEnvs 建议换新动作时先从大的试，模仿跟不上再往小调——这条病就是「调过了头」。

<h3 id="病历-模式崩塌">④ 模式崩塌</h3>

- **所有常用对抗曲线都可以看起来健康**：$r^S$ 在涨、$D$(假) 往 0 爬、准确率在降。
- 揭穿它的是**速度覆盖**（卡在 ~0.40）或回放：策略只会数据集里最好学的那种走，跑步 / 转向都不会。
- **原因**：混合数据集里各片段难度差太大；没有多样性压力。
- **处理**：检查数据集是否被走路片段占满；打开回放；ASE 会在 AMP 上再加多样性奖励。这条病 TensorBoard 默认列上看不出来。

<h3 id="病历-任务压垮风格">⑤ 任务压垮风格</h3>

- $r^G$ 可以到 0.92，速度覆盖也铺开。
- $r^S$ 贴地，$D$(假) 停在 −0.7，准确率降不下来——判别器一直轻易分清。
- 回放：能跟住 $v^{\ast}$，动作是怪的（滑步、直腿）。
- **原因**：$w^G$ 太大，或风格项没接到合成奖励。和 DeepMimic 的「只有任务」、PPO 的「奖励在骗你」是同一类事故。
- **处理**：把 $w^S$ 拉回 0.5；从第一天起同时记 $r^S$ 和 $r^G$，不要只盯总分。

<h3 id="病历-只有风格">⑥ 只有风格</h3>

- $r^S$ 和 $D$ 走健康曲线，回放姿态也对。
- $r^G$ 停在 ~0.20，速度覆盖停在数据集的几个峰上，跟不住随机 $v^{\ast}$。
- **原因**：MimicKit 默认 `task_reward_weight: 0.0`（纯 disc reward）；或者 Target Heading 的 $w^G$ 忘了拨。和 DeepMimic Table 4 的「仅模仿 → Strike 19%」是同一类事故。
- **处理**：加回任务项（论文 $w^S=w^G=0.5$）。不要指望「风格够像任务就会顺带做成」。

> 🔑 **三条纪律**：① 用平滑窗口看趋势，不要被单次迭代吓到；② $r^S$ 异常时先看 $D$(假) 和准确率，再动 GP；③ 对抗曲线好看不等于步态多样、任务做成，训练后期每天看一眼回放。PPO 的更新过大 / 几乎没学 / Critic 失灵，在这篇里同样成立，读法见 PPO 笔记。
...
<details class="paper-fold" markdown="1">
<summary>📖 展开全文（6 节）：Q1: AMP 和 DeepMimic 的核心区别？…</summary>
...
**A**: 和 GAN 一样的问题：①模式崩塌——策略只学会参考数据中的一种动作；②训练不稳定——鉴别器太强或太弱都会导致策略学不好。论文用了梯度惩罚（gradient penalty）来稳定训练。六种病历的曲线形态见 [训练曲线怎么读](#amp-curves-guide)。
...
**A**: 算法侧复用 PPO，KL / clip / 熵的读法见 PPO 笔记（AMP 的 clip 是 **0.02**）。这篇多出来的是判别器侧：① **风格奖励 $r^S$**（从 0 爬到约 0.75，不是 1；$D=0$ 时代入 LSGAN 式正好是 0.75）；② **$D$(假)/$D$(真)**（假从 −1 到 0，真从 1 降到 ~0.35）；③ **agent / demo 准确率**（应从 ~99% 降下来；长期 99% 是判别器过强，长期 50% 是过弱）；④ **梯度惩罚**（看区间 ~1；IsaacGymEnvs 说换新动作最先调 `disc_grad_penalty`）。$r^S$ 贴地先看 $D$(假) 是不是钉在 −1；一上来就 0.75 先看准确率是不是两边 50%；对抗曲线都健康仍要打开回放 —— 可能是模式崩塌。详见 [训练曲线怎么读](#amp-curves-guide)。
</details>
