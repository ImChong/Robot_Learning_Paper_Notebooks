...
> 1. 判别器 loss 实验台 —— 正文那 4 个样本的数值例子，改一个打分看两项 loss 怎么动
> 2. 判别器打分 → 风格奖励 —— LSGAN 形式和 log 形式画在一起，一眼看出论文为什么换了形式
> 3. 风格 × 任务 —— 同一个任务奖励换数据集就换风格，还能看到「混合数据集才有步态切换」
...
下面这个演示把"运动"压成一个标量（这一步迈多快），但对抗训练那一环是真的在浏览器里跑：判别器按 LSGAN 目标在参考数据和策略样本之间训练，风格奖励再喂回策略。任务奖励用的就是论文 Target Heading 的原式 $r^G = \exp(-0.25(v^* - v)^2)$：
...
> 🎮 上面那个演示的右图就是这件事的最小复现：把 $v^*$ 从 0.6 扫到 5，走路数据集在高速段拉不上去、跑步数据集在低速段下不来，只有混合数据集能大致跟住对角线——而且在 $v^* \approx 2.4$ 附近会看到一次明显的**跳变**，那就是步态切换。
