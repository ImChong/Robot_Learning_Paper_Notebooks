...
## 📈 训练曲线怎么读 {#deepmimic-curves-guide}

算法侧直接复用 PPO，所以 KL / clip / 熵怎么读见 [PPO 笔记的训练曲线](../PPO_Proximal_Policy_Optimization/PPO_Proximal_Policy_Optimization.html#ppo-curves-guide)。DeepMimic 每天多盯的是**环境侧**那几条：归一化回报、四维分项、ET 率、相位覆盖。多数线不能用「越高越好」打发——加权总分会把末端和质心的窟窿盖住，存活步数在没开 ET 时甚至会骗人。下面这个实验台把 Humanoid Backflip 的示意曲线做成六种病历：先点「健康」记住标准形态，再点坏掉的那几种，看是哪条线先报警。

<div class="paper-demo" data-demo="deepmimic-curves"><p class="demo-fallback">（本节含交互演示，需要启用 JavaScript）</p></div>

### 方向速查表

带 \* 的「好方向」有前提：模仿权重没写歪、回放你真的看过。表格里的健康终点按 Section 10.4 的 Backflip（RSI+ET = **0.791**）；Table 2 发表值是 0.729，评测设定略有不同，数量级一致。PPO 那几条线的列名见 PPO 笔记。

| 曲线 | 好方向 | 健康形态 | 立刻要查的坏样子 |
|------|--------|----------|------------------|
| **归一化回报** | 越来越高\* | S 形：0.08 → 0.20 → 0.45 → 0.66 → **0.791** | 横盘在 ~0.2；停在 0.73 只会后跳；腰斩到 0.38 |
| **四维分项** | 四条都爬，别只看加权和 | $r^p$ 最重；$r^e$（$k=40$）最严、爬得最慢 | 总分好看但 $r^e$/$r^c$ 停在 0.3 以下 |
| **ET 率** | 从接近 1 降下来 | 前期 ~0.94，后期 ~0.14 | 长期 ≈ 1（还在秒摔）；长期 = 0（开关没开） |
| **存活步数** | 本任务：拉到 clip 长 | Backflip 1.75 s × 30 Hz ≈ **53 步** | 一直几步（秒摔）；很长但回报低（在地上挣扎） |
| **相位覆盖** | 七个阶段铺匀 | RSI 开着时接近均匀 | 全压在助跑 / 蓄力（没开 RSI） |
| **任务成功率** | 有 $r^G$ 的任务才看 | Strike 两者都有时 **99%** | 模仿分很高但 Strike 停在 **19%** |

> ⚠️ **MimicKit 默认日志比论文瘦**：环境侧只给你逐步奖励之和。四维分项、ET 率、相位直方图要在 `compute_reward()` / `compute_done()` / `sample_time()` 里自己记，否则「权重失衡 / 没开 RSI」这两种病要等回放才看得见。

### 先看哪几条，再怎么交叉验证

优先级从高到低只有三层：

1. **动作成不成**：归一化回报 + 回放。这条不涨，后面全是噪声。
2. **样本花在哪**：ET 率 + 相位覆盖。回报一旦横盘，先来这里找原因。
3. **像不像、任务做没做**：四维分项 + 任务成功率。它们揭穿「总分好看但脚在滑」和「只会模仿不会扔球」。

<div class="mermaid">
flowchart TB
    R{"回报在涨 / 横盘 / 腰斩？"}
    R -->|横盘在 ~0.2| P0{"相位是不是全压在前两段？"}
    P0 -->|是| NoRsi["没开 RSI：只会助跑"]
    P0 -->|不是，ET 一直为 1| Fall["还在秒摔：先查 ET 和观测"]
    R -->|停在 ~0.38| Et{"ET 率是不是一直为 0？"}
    Et -->|是| NoEt["没开 ET：地上挣扎污染 batch"]
    Et -->|不是| Ppo["去看 PPO 的 KL / clip"]
    R -->|看起来在涨| Vid{"打开回放：真的在翻？"}
    Vid -->|只会小幅后跳| NoRsi2["没开 RSI：对照 0.730"]
    Vid -->|脚滑 / 质心漂| Imb["权重失衡：拆开四维"]
    Vid -->|姿态对但任务没成| Imit["只有模仿：Strike 19%"]
    Vid -->|任务成了但动作丑| Task["只有任务：补回 r^I"]
    Vid -->|是| OK["健康：对照 0.791"]
</div>

<details class="paper-fold" markdown="1">
<summary>📖 展开文字：每条曲线的含义、健康区间、坏样子和原因</summary>

<h3 id="曲线-归一化回报">归一化回报（论文主指标）</h3>

把一个 episode 的逐步奖励加总，再除以「运动学角色按参考 clip 走一遍」拿到的回报，满分是 1。Section 10.4 的 Backflip 健康终点是 **0.791**；Table 2 发表值是 0.729。示意锚点和第 4 步那张表同一组：

| 迭代 | 0 | 500 | 2000 | 3500 | 5000 |
|------|---|-----|------|------|------|
| 归一化回报 | 0.08 | 0.20 | 0.45 | 0.66 | 0.791 |

- **好方向**：越来越高。前期慢是各相位都在摔，中期陡是片段能做了，后期平台是落地焊上。
- **横盘在 ~0.2**：策略还是随机的。查学习率、观测是否带了相位 $\phi$、参考动作有没有加载对。
- **停在 0.730**：Section 10.4 的「仅 ET」——没有 RSI，策略只会「小幅向后跳」，空翻后半段从没练过。
- **腰斩到 0.379**：同一张表的「仅 RSI」——ET 没开，地上挣扎的状态占满 batch。
- **涨得又快又稳，视频却难看**：进入「权重失衡」或「只有任务」，这条曲线单独看会给出假阳性。

<h3 id="曲线-四维分项">四维分项（$r^p, r^v, r^e, r^c$）</h3>

论文的模仿奖励是四项指数加权和（MimicKit 实现里拆成五项，多了 `root_pose` / `root_vel`，读日志时对上即可）：

$$r_t^I = 0.65\,r_t^p + 0.1\,r_t^v + 0.15\,r_t^e + 0.1\,r_t^c$$

- **$r^p$（$k=2$）**：权重最大，决定动作整体对不对。它先爬是正常的。
- **$r^v$（$k=0.1$）**：最宽松，往往一上来就不低——别把它当「已经学会了」。
- **$r^e$（$k=40$）**：最严。手脚差几厘米就掉分，健康训练里它爬得最慢，也最不该被总分盖住。
- **$r^c$（$k=10$）**：质心漂了动作再像也站不稳。
- **总分好看、$r^e$/$r^c$ 停在 0.3 以下**：权重失衡。$w_p=0.65$ 能把窟窿填平，回放里常见脚滑、穿地、质心后坐。
- **四条一起不涨**：奖励尺度或参考帧对不上，先打印一步的四项误差。

<h3 id="曲线-et率">ET 率（摔倒被截断的比例）</h3>

一个 batch 里 `compute_done()` 判为 FAIL 的比例。Backflip 这种高动态技能，前期接近 1 是正常的。

- **好方向**：从 ~0.94 降到 ~0.14。后期还留一点，是落地不稳或偶发偏离。
- **长期 ≈ 1**：还在秒摔。和回报横盘在 0.2 是同一件事。
- **长期 = 0**：不是「从不摔」，是 **ET 开关没开**。此时存活步数会被拉满，看起来像「活得很好」，其实是在地上挣扎。
- MimicKit 的 pose termination 还加了一条：任一 body 偏离参考超过 1.0 m 也会 FAIL。所以 ET 率高不一定是摔了，也可能是飞出去了。

<h3 id="曲线-存活步数">存活步数（先搞清终止条件）</h3>

非循环技能的 episode 长度等于动作片段长度。Backflip 1.75 s、控制 30 Hz，上限大约 **53 步**。循环技能（Walk）论文设成 20 s。

- 一直几步：还在秒摔。
- 很快到顶、回报却不高：两种病长得像——① 没开 ET，躺地上把步数耗完；② 学会了站着 / 小跳，没学会空翻。用相位覆盖和回放区分。
- 长度抖动很大：并行环境里有的已经能翻、有的还在摔，属于中期正常噪声。

<h3 id="曲线-相位覆盖">相位覆盖（RSI 有没有在干活）</h3>

把参考 clip 切成七段（助跑 → 蓄力 → 起跳 → 团身 → 倒立 → 展开 → 落地），统计每个 episode 实际练到了哪一段。

- **RSI 开着**：每次 reset 均匀抽 $t_0$，七段接近均匀。即使前期秒摔，摔的也是「随机相位上的第一步」，落地也能被练到。
- **RSI 关着**：每个 episode 从帧 0 起。前期只撑得住前两段，柱状图递减——和上面那个 RSI 消融演示右边那张图是同一件事。
- **Walk 为什么无所谓**：低动态很少摔，不靠 RSI 也能走到后面的相位。Q7 表里 Walk 0.980 / 0.981 / 0.974 三列几乎一样。

<h3 id="曲线-任务成功率">任务成功率（有 $r^G$ 才看）</h3>

总奖励 $r = 0.7\,r^I + 0.3\,r^G$。纯模仿技能（Backflip）没有独立的任务成功率；Strike / Throw 这种才要单独记。
...
- 模仿分很高、成功率停在 19%：策略在「像不像」上卷，没被导向击打。
- 成功率很高、四维分项很低：任务做成了，动作是怪的（抱着球跑而不是扔）。
- 纯模仿技能不要硬造一条「成功率」——用归一化回报和回放就够。

<h3 id="曲线-名字对照">名字对照：你日志里那一列到底是谁</h3>

| 本文称呼 | 论文 | MimicKit |
|----------|------|----------|
| 归一化回报 | Table 2 / Section 10.4 | `test_model()` 的评估回报（需自己除以 clip 满分） |
| 四维分项 | $r^p, r^v, r^e, r^c$ | `compute_reward()` 的 pose / vel / key_pos / root_* ，默认只返回加权和 |
| ET 率 | Early Termination | `compute_done()` 的 `DoneFlags.FAIL` 比例 |
| 存活步数 | episode 长度 = clip 长（非循环） | 需自己记 |
| 相位覆盖 | RSI 的均匀采样 | `MotionLib.sample_time()` 的直方图 |
| 任务成功率 | Table 4 Strike / Throw | 任务项 $r^G$ 的成功判定，需自己加 |
| KL / clip / 熵 | 标准 PPO | 见 PPO 笔记；源码默认只打包 `critic_loss` |
...
<h3 id="病历-健康">① 健康（RSI + ET）</h3>

- 回报：0.08 → 0.20 → 0.45 → 0.66 → **0.791** 的 S 形（第 4 步那张表）。
- 四维分项都在爬；$r^e$ 最慢，但不会停在 0.3 以下。
- ET 率从 ~0.94 降到 ~0.14；存活步数拉到 ~53。
- 七个相位接近均匀。
- **什么都不用改。** 打开回放确认落地，存 checkpoint。

<h3 id="病历-关掉rsi">② 关掉 RSI</h3>

- **先兆**：相位柱状图递减，落地几乎为 0。
- **发作**：回报还能涨到 **0.730**（Section 10.4「仅 ET」），看起来不惨，回放却是「小幅向后跳」——助跑和起跳会了，团身之后没人练。
- **原因**：每个 episode 从帧 0 起步，前期秒摔，后面的相位轮不到。Walk 上这个开关几乎没影响（0.980 vs 0.981）。
- **处理**：打开 RSI（`MotionLib.sample_time()` 均匀抽相位）。已经训歪的 checkpoint 回滚比硬补后半段容易。

<h3 id="病历-关掉et">③ 关掉 ET</h3>

- 回报腰斩到 **0.379**（Section 10.4「仅 RSI」），比关掉 RSI 惨得多。
- ET 率钉在 0——不是不摔，是摔了也不停。存活步数被拉满，看起来像「活得很久」。
- 四维分项爬不动：batch 里大量「躺在地上扭」的状态，class imbalance。
- **原因**：`enable_early_termination: false`，或摔倒检测没把非脚接触算进去。
- **处理**：打开 ET；确认 `not_first_step = (time > 0.0)` 还在，避免第一步误杀。Q7 的数字：Backflip 仅 RSI 0.379，仅 ET 0.730，两个开关要一起开。

<h3 id="病历-权重失衡">④ 权重失衡</h3>

- 归一化回报可以涨到 0.65+（$w_p=0.65$ 把总分撑住了）。
- **拆开四维**：$r^p$、$r^v$ 已经很高，$r^e$、$r^c$ 停在 ~0.25。
- 回放：关节角度对、脚在滑、质心后坐。
- **原因**：$w_e$/$w_c$ 太小，或 $k_e$ 被改松；有人只盯加权和做 early stop。
- **处理**：分开画四条线；把 $w_e$ 从 0.15 往上试，或确认 $k_e=40$ 没被改掉。这是简化模型，数值不能和论文直接比。

<h3 id="病历-只有模仿">⑤ 只有模仿</h3>

- $r^I$ 看起来健康，四维分项也在爬。
- **任务成功率不动**：Strike **19%**、Throw **5%**（Table 4），两者都有时是 99% / 75%。
- **原因**：$\omega_G=0$ 或任务奖励没接到 `compute_reward()`。纯模仿技能（Backflip）没有这条病；Strike / Throw 才有。
- **处理**：加回 $r^G$（论文 $\omega_I=0.7$、$\omega_G=0.3$ 对所有任务共用）。不要指望「模仿够像任务就会顺带做成」。

<h3 id="病历-只有任务">⑥ 只有任务</h3>

- 任务成功率可以很高，归一化模仿回报却停在 ~0.25。
- 四维分项全低：策略找到了怪异但功能性的解（抱着球跑、脚滑着冲向目标）。
- **原因**：$\omega_I=0$ 或四维权重全是 0。和 PPO 笔记里「奖励在骗你」是同一类事故，只是这里骗你的不是 alive bonus，是任务项。
- **处理**：加回 $r^I$；从第一天起同时记模仿分和任务成功率。曲线好看不等于动作自然。

> 🔑 **三条纪律**：① 用平滑窗口看趋势，不要被单次迭代吓到；② 回报异常时先看相位覆盖和 ET 率，再动四维权重；③ 加权总分好看不等于手脚对、任务做成，训练后期每天看一眼回放。PPO 的更新过大 / 几乎没学 / Critic 失灵，在这篇里同样成立，读法见 PPO 笔记。
...
| 阶段 | 迭代 | 归一化回报（示意） | 表现 |
|------|------|-------------------|------|
| **初期** | 0–500 | 0.08 → 0.20 | 从各个阶段开始都会摔倒 |
| **学会片段** | 500–2000 | 0.20 → 0.45 | 能完成部分片段（如落地、起跳），但串不起来 |
| **完成全程** | 2000–5000 | 0.45 → 0.66 | 能从头到尾完成后空翻，但有时落地不稳 |
| **精细化** | 5000 | **0.791** | 后空翻流畅自然，落地稳定（Section 10.4 的 RSI+ET） |

把这张表画成学习曲线，能看到高动态技能的典型形态——前期爬升慢（各相位都在摔），中期片段能做了但串不起来，后期才把空翻和落地焊上。真正训练时日志上不止「会不会摔」这一条；归一化回报、四维分项、ET 率、相位覆盖怎么读、坏了是什么病，见 [训练曲线怎么读](#deepmimic-curves-guide)。

<div class="mermaid">
xychart-beta
    title "Humanoid Backflip 归一化回报（示意）"
    x-axis "迭代次数" [0, 500, 2000, 3500, 5000]
    y-axis "归一化回报" 0 --> 1
    line [0.08, 0.20, 0.45, 0.66, 0.79]
</div>
...
<details class="paper-fold" markdown="1">
<summary>📖 展开全文（11 节）：Q1: DeepMimic 的核心创新是什么？ / Q2: 模仿奖励为什么用指数函数形式？…</summary>
...
<h3 id="q11-训练日志上该看哪些曲线越高越好吗">Q11: 训练日志上该看哪些曲线？越高越好吗？</h3>
**A**: 算法侧复用 PPO，KL / clip / 熵的读法见 PPO 笔记。这篇多出来的是环境侧四条：① **归一化回报**（越高越好，Backflip 健康终点约 0.791）；② **四维分项**（$r^p/r^v/r^e/r^c$ 都要爬，不能只看加权总分——$w_p=0.65$ 会把末端和质心的窟窿盖住）；③ **ET 率**（应从 ~0.94 降到 ~0.14；长期为 0 是开关没开，不是从不摔）；④ **相位覆盖**（七个阶段要铺匀，否则没开 RSI）。回报横盘先看相位是不是全压在助跑、以及 ET 是不是一直为 1；回报好看但回放难看，拆开四维——末端和质心掉了就是权重失衡，Strike 停在 19% 就是只有模仿，姿态项很低但任务成了就是只有任务。详见 [训练曲线怎么读](#deepmimic-curves-guide)。
</details>
...
| 21 | 训练 DeepMimic 时日志上该先看哪几条曲线？ | 先看归一化回报和回放（动作成不成），再看 ET 率和相位覆盖（样本花在哪），最后拆开四维分项和任务成功率（像不像、任务做没做）。KL / clip / 熵见 PPO 笔记。 |
