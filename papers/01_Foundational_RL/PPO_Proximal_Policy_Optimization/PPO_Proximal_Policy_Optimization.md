...
带 \* 的「好方向」有前提：奖励函数没写歪、终止条件你真的理解。表格里的健康区间按上面 Humanoid 示例（$\varepsilon=0.2$，rsl_rl 常见 `desired_kl ≈ 0.01`）。

| 曲线 | 常见名字 | 好方向 | 健康形态 | 立刻要查的坏样子 |
|------|----------|--------|----------|------------------|
| **回合回报** | `ep_rew_mean` / `Mean reward` | 越来越高\* | S 形：前期慢 → 中期陡 → 后期平台 | 突然腰斩；长期横盘在随机水平（~50） |
| **回合长度** | `ep_len_mean` / `Mean episode length` | 本任务：通常越来越长 | 从几十步拉到超时上限 1000 | 一直 ~50 步（还在秒摔）；到顶了但回报不高（站着混存活奖） |
| **近似 KL** | `approx_kl` / `KL` | **看区间，不是越高越好** | 稳定在 ~0.01（大约 0.008–0.025） | 持续高于 0.05；尖峰后回报崩；长期 ≈ 0 |
| **clip 比例** | `clip_fraction` / clipfrac | **看区间** | 前期约 30%，千轮约 10%（见[附录 E](#e-训练过程中各组件的变化)） | 长期高于 40%；长期 ≈ 0 |
| **策略熵 / $\sigma$** | `entropy` / `std` | 慢慢变低，别到 0 | 平滑下降，收在一个还在探索的平台 | 几百轮就塌到 0；训练后期仍钉在初始值 |
| **解释方差** | `explained_variance` / EV | 越来越接近 1 | 从负/0 爬到 0.7+ | 长期为负（还不如猜均值） |
| **价值损失** | `value_loss` / `critic_loss` | 总体走低，允许随回报抬升反弹 | 先降后稳；回报从 50 涨到 5000 时绝对值可以回升 | 爆炸；很快到 0 但回报不动 |
| **策略损失** | `policy_loss` / `surrogate` | **不要按高低判断** | 在 0 附近晃 | 绝对值爆炸；或长期钉死在 0 |
...
flowchart TB
    R{"回报在涨、还是横盘、还是腰斩？"}
    R -->|横盘在随机水平| K0{"KL / clip 是不是都接近 0？"}
    K0 -->|是| Tiny["几乎没学：lr 太小、ε 过紧、观测没归一化"]
    K0 -->|否，KL 很大| BigEarly["步子已经迈飞了，只是回报还没来得及崩"]
    R -->|突然腰斩| K1{"腰斩前 KL 有没有冲出 0.05？"}
    K1 -->|有| Big["更新过大：降 lr / actor_epochs，回滚 checkpoint"]
    K1 -->|没有，但熵已经到 0| Ent["熵塌缩：卡在站立等局部最优"]
    R -->|看起来在涨| Vid{"打开仿真回放：机器人真的在做任务吗？"}
    Vid -->|没有，在刷存活奖 / 空转| Hack["奖励在骗你：改奖励或盯前进速度这类任务指标"]
    Vid -->|有| EV{"解释方差在往 1 爬吗？"}
    EV -->|长期为负| Critic["Critic 失灵：先把 V 拟合好"]
    EV -->|在爬| OK["健康：继续训练，开始看后期平台"]
</div>
