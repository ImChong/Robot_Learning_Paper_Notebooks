...
笔记里可以内嵌可交互的小演示（滑块 / 拖拽 / 浏览器内跑的小实验）。`papers/01_Foundational_RL/` 下的 **15 篇笔记已全部接入**，每篇 3 个演示（PPO / AWR / DeepMimic / AMP / ADD / PHC / ASE / CALM / PULSE / Diffusion Policy / BeyondMimic 额外各多一个 `*-explainer` 讲解动画，播放器是 `kit.js` 的 `K.explainer`），bundle 与笔记一一对应（`ppo` / `awr` / `deepmimic` / `amp` / `add` / `ase` / `calm` / `pulse` / `phc` / `diffusion_policy` / `beyondmimic` / `lcp` / `domain_randomization` / `dr_theory` / `mimickit`）。板块 01 之外目前有五篇接了演示：`papers/03_High_Impact_Selection/SONIC.../` 的 `sonic` bundle、`papers/03_High_Impact_Selection/GR00T_N1.../` 的 `groot` bundle、`papers/03_High_Impact_Selection/Cosmos.../` 的 `cosmos` bundle、`papers/02_Motion_Retargeting/Retargeting_Matters.../` 的 `gmr` bundle，与 `papers/02_Motion_Retargeting/OmniRetarget.../` 的 `omniretarget` bundle，五者都只有一段 `*-explainer` 讲解动画。
