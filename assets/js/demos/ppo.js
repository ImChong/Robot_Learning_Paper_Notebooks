...
(function () {
  ...
  function buildClipDemo(host) {
    var root = card(host, {
      ...
      sub: '横轴是概率比 $r = \\pi_\\theta(a \\mid s) / \\pi_{old}(a \\mid s)$，纵轴是这条样本贡献的目标值。三条线分别是未裁剪项 $r\\hat{A}$、裁剪项 $\\mathrm{clip}(r)\\,\\hat{A}$，以及 PPO 真正优化的 $\\min$（两者）。'
    });
    ...
    note(root, [
      '**怎么读**：只要 min 选中的是「顶在边界上的裁剪项」，它对 $\\theta$ 就是常数，梯度为 0 —— 这条样本本轮不再推动策略，即笔记里说的**冻结**。',
      '**容易被忽略的一半**：冻结只发生在「往外冲」的那一侧。好动作 $\\hat{A}>0$ 时只有 $r>1+\\varepsilon$ 会冻；**坏动作 $\\hat{A}<0$ 时 $r>1+\\varepsilon$ 反而梯度最大**（min 选中未裁剪项），把已经跑偏的策略狠狠拉回来。把 $\\hat{A}$ 拖到负数、$r$ 拖到 1.6 就能看到这个「回拉区」。',
      '**$\\varepsilon$ 在调什么**：把 $\\varepsilon$ 从 0.05 拖到 0.5，安全带变宽，允许单轮走更大的步子，冻结来得更晚 —— 更快，也更容易一步走过头。'
    ]);
    ...
    var render = registerRenderer(function () {
      ...
      if (regime === 'frozen') {
        msg =
          state.adv >= 0
            ? 'r = ' + fmt(state.r, 2) + ' > $1+\\varepsilon$ = ' + fmt(hi, 2) + '：好动作已经被抬到安全带之外，min 选中常数 ' + fmt(hi, 2) + ' $\\hat{A}$，梯度为 0 —— 本轮不再加码。'
            : 'r = ' + fmt(state.r, 2) + ' < $1-\\varepsilon$ = ' + fmt(lo, 2) + '：坏动作已经被压到安全带之外，min 选中常数 ' + fmt(lo, 2) + ' $\\hat{A}$，梯度为 0 —— 本轮不再狂砍。';
        verdict.set('🧊 冻结：' + msg, 'frozen');
      } else if (regime === 'pullback') {
        msg =
          state.adv >= 0
            ? '好动作却只有 r = ' + fmt(state.r, 2) + ' < $1-\\varepsilon$：min 选中未裁剪项，梯度 = $\\hat{A}$ = ' + fmt(state.adv, 2) + ' > 0，把概率往回抬。'
            : '坏动作却有 r = ' + fmt(state.r, 2) + ' > $1+\\varepsilon$：min 选中未裁剪项（更悲观的那个），梯度 = $\\hat{A}$ = ' + fmt(state.adv, 2) + ' < 0，把跑偏的概率狠狠压回来。';
        verdict.set('↩️ 回拉区（裁剪不挡这一侧）：' + msg, 'learning');
      } else {
        verdict.set(
          '✅ 正常学习：r = ' + fmt(state.r, 2) + ' 在安全带 [' + fmt(lo, 2) + ', ' + fmt(hi, 2) + '] 内，两项相等，梯度 = $\\hat{A}$ = ' + fmt(state.adv, 2) + '，' + (state.adv >= 0 ? '继续抬高该动作概率。' : '继续压低该动作概率。'),
          'learning'
        );
      }
  ...
  function buildGaeDemo(host) {
    var root = card(host, {
      title: 'GAE 实验台：$\\lambda$ 怎么把「4 步后的那一摔」传回 $t=0$',
      sub: '改奖励、改 Critic 的 $V(s)$、拖 $\\gamma$ 和 $\\lambda$，下面的 $\\delta_t$ 与 $\\hat{A}_t$ 立刻重算。$\\lambda=0$ 是一步 TD（短视、低方差），$\\lambda=1$ 是蒙特卡洛（看到底、高方差）。'
    });
    ...
    note(root, [
      '**先看最左边那根柱子**：默认场景里 $t=0$ 只看一步（$\\lambda=0$）时 $\\delta_0 = +0.5$，像是「走得挺好」；把 $\\lambda$ 拖到 0.95，$\\hat{A}_0$ 掉到 −49 左右 —— $\\lambda$ 让 $t=0$ 提前看见 4 步后的那一摔，责任被往前传。',
      '**$\\lambda$ 到底在权衡什么**：$\\lambda$ 越大越依赖真实发生的后续奖励（偏差小、方差大），越小越依赖 Critic 的 $V$ 估计（方差小、但 Critic 估错就跟着错）。切到「Critic 提前预警」预设：$V$ 本身已经一路下滑，一步 TD 就能看出 $t=0$ 不妙，符号不再被 $\\lambda$ 翻转 —— $\\lambda$ 要补的，正是 Critic 没看出来的那部分。',
      '**第三个预设「Critic 估得准」**：$V(s_t)$ 正好等于后面真实拿到的折扣回报，于是每个 $\\delta_t$ 都接近 0、$\\hat{A}_t$ 也接近 0。**优势 $\\approx 0$ 不代表走得差**，只代表「和该状态的平均水平持平」，这批数据对策略几乎没有修正意见 —— 这正是训练后期的常态。',
      '**为什么 done 处要截断**：最后一步是轨迹边界，$\\delta$ 不再 bootstrap 下一个 $V$，递推也从这里重新起步 —— 否则会把下一条轨迹（重置后的新 episode）的信息错误地传回来。'
    ]);
    ...
    var render = registerRenderer(function () {
      ...
      if (flips) {
        verdict.set(
          '⚠️ 符号被 $\\lambda$ 翻转了：一步 TD 认为 $t=0$ 是好动作（$\\hat{A}_0$ = ' + fmt(advZero[0], 1) + '），当前 $\\lambda$ = ' + fmt(state.lam, 2) + ' 却认为是坏动作（$\\hat{A}_0$ = ' + fmt(advCur[0], 1) + '）。裁剪表格里「好动作 / 坏动作」分在哪一列，取决于这一步。',
          'frozen'
        );
      } else {
        verdict.set(
          '$\\lambda$ = ' + fmt(state.lam, 2) + ' 时 $\\hat{A}_0$ = ' + fmt(advCur[0], 1) + '，介于一步 TD 的 ' + fmt(advZero[0], 1) + ' 和蒙特卡洛的 ' + fmt(advOne[0], 1) + ' 之间 —— GAE 就是这两端的加权插值。',
          'learning'
        );
      }
  ...
  function buildEpochsDemo(host) {
    ...
    note(root, [
      ...
      '**右图是「刹车」本身**：横轴是同一批数据的第几个 epoch，纵轴是这批样本平均偏离旧策略多远（平均 $|r-1|$）。epoch 0 时 $r$ 恒等于 1；之后**无裁剪那条线一路爬到 1 以上**（新策略已经和采样时的旧策略完全是两回事，重要性采样早就不成立了），**PPO-Clip 那条线则被摁在 $\\varepsilon$ 附近来回蹭** —— 这就是自动踩下的刹车。把「右图看第几轮迭代」往后拖，会看到策略收敛后连刹车都不怎么用得上了。',
